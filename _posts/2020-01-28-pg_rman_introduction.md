...
백업 솔루션의 구체적인 사용법을 익히기 전에 DML 트랜잭션이 발생하면 그 내용(커밋 내용, 인덱스 포인트 정보, 체크섬 등을 포함한 해당 오퍼레이션의 다양한 정보)이 처리되는 과정을 복습하고 checkpoint, timeline 같은 개념을 되새겨보고 pg_rman 사용법을 익히는 과정으로 나아가자.  
...
checkpoint_completion_target 설정에 따른 디스크 I/O 변화를 [테스트한 글](https://www.depesz.com/2010/11/03/checkpoint_completion_target/)을 참고하면 좋을 것 같다. 아래 조건에서 테스트가 진행됐다.  
...
tar xvfz /archive/arc_wal/data/db_backup_date*.tar.gz -C /var/lib/edb/as11/data
